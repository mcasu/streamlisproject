...
function Gab(serverAddress, token, chatId){
  ...
  /**
   * @property {function} onVideoType Parameter: senderId, video type message.
   * @memberOf Gab#
   */
  this.onVideoType=null;
  /**
  ...
  var connect=function(serverAddress, token, chatId){
    ...
    wsServer.on('video-type',function(data){
      console.log('Received video type message');
      if(self.onVideoType)
        self.onVideoType(data.from, data.data);
    });
   ...
   */
  this.sendVideoType=function(uid, stream){
    wsServer.emit('video-type',{to:uid, data:stream});
  };
